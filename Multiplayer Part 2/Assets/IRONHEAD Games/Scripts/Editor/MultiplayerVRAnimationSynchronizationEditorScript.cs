...
[CustomEditor(typeof(MultiplayerVRAnimationSynchronization))]
public class MultiplayerVRAnimationSynchronizationEditorScript : Editor
{
    private Animator m_Animator;
    private MultiplayerVRAnimationSynchronization m_Target;
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (this.m_Animator == null)
        {
            ...
            this.m_Target = (MultiplayerVRAnimationSynchronization)this.target;
            this.m_Animator = this.m_Target.m_Animator; 
            //return;
        }
    ...
    private void DrawWeightInspector()
    {
        ...
        for (int i = 0; i < this.GetLayerCount(); ++i)
        {
            ...
            {
                this.m_Target.SetLayerSynchronized(i, MultiplayerVRAnimationSynchronization.SynchronizeType.Disabled);
            }

            MultiplayerVRAnimationSynchronization.SynchronizeType syncType = this.m_Target.GetLayerSynchronizeType(i);
            ...
            syncType = (MultiplayerVRAnimationSynchronization.SynchronizeType)EditorGUI.EnumPopup(popupRect, syncType);
    ...
    private void DrawParameterInspector()
    {
        ...
        for (int i = 0; i < this.GetParameterCount(); i++)
        {
            ...
            {
                this.m_Target.SetParameterSynchronized(parameter.name, (MultiplayerVRAnimationSynchronization.ParameterType)parameter.type, MultiplayerVRAnimationSynchronization.SynchronizeType.Disabled);
            }

            MultiplayerVRAnimationSynchronization.SynchronizeType value = this.m_Target.GetParameterSynchronizeType(parameter.name);

            // check if using trigger and actually synchronizing it
            if (value != MultiplayerVRAnimationSynchronization.SynchronizeType.Disabled && parameter.type == AnimatorControllerParameterType.Trigger)
            {
            ...
            Rect popupRect = new Rect(elementRect.xMin + EditorGUIUtility.labelWidth, elementRect.yMin + 2, elementRect.width - EditorGUIUtility.labelWidth - 5, EditorGUIUtility.singleLineHeight);
            value = (MultiplayerVRAnimationSynchronization.SynchronizeType)EditorGUI.EnumPopup(popupRect, value);
            ...
            if (value != this.m_Target.GetParameterSynchronizeType(parameter.name))
            {
                ...
                this.m_Target.SetParameterSynchronized(parameter.name, (MultiplayerVRAnimationSynchronization.ParameterType)parameter.type, value);
            }
        }
